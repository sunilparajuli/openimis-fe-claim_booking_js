...
export function fetchClaimBookings(mm, prms) {
  const filters = { ...prms };

  if (prms.title_Icontains) {
    filters.title_Icontains = prms.title_Icontains;
  }

  // Format the GraphQL payload
  ...
  return graphql(payload, "FETCH_BOOKINGS");
}
...
export function createClaimBooking(mm, notice, clientMutationLabel, clientMutationDetails = null) {
  let noticeGQL = `
    title: "${notice.title}"
...
export function updateClaimBooking(mm, notice, clientMutationLabel, clientMutationDetails = null) {
